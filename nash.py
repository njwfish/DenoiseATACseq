...
from keras.layers import Conv2D, Dense, Reshape, GRU
        ...
        for i in range(self.steps):
            ...
            for j in range(len(parent.layers) - 1):  # Cannot modify last layer
                if isinstance(parent.layers[j], (Conv2D, Dense)):
                    ...
                    if j != len(parent.layers) - 1 and not isinstance(parent.layers[j], Reshape) and not isinstance(parent.layers[j], GRU):
                        continue
